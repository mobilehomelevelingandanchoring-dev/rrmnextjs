...
- `services.ts` — 8 services (pressure-washing, gutter-cleaning, etc.) with SEO metadata, FAQs, LSI keywords
...
2. Register it in `ServiceDynamicClient.tsx` under the correct service+location slug
3. Add location-specific metadata to the `locationMeta` object in `app/services/[...slug]/page.tsx`
4. Update `app/sitemap.ts` if the route isn't already generated
